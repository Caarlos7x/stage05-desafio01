...
function countDown() {
  timeOut = setTimeout(() => {
    ...
    updateaDisplay(currentMinutes, currentSecons - 1);
    countDown();
  }, 1000);
}
...
function increaseFiveMinutes() {
  if (!countDownIsRunning) {
    updateaDisplay((initialMinutes += 5), 0);
    updateInitialMinutes();
    return;
  }
  if (currentMinutes === undefined) {
  ...
  updateaDisplay((currentMinutes += 5), currentSecons - 1);
}
...
function stop() {
  countDownIsRunning = false;
  clearTimeout(timeOut);
  updateaDisplay(initialMinutes, 0);
}
function pause() {
  clearTimeout(timeOut);
}
function updateaDisplay(minutes, seconds) {
  displayMinutes.textContent = String(minutes).padStart(2, "0");
  displaySeconds.textContent = String(seconds).padStart(2, "0");
}
